-- Update compute_chantier_metrics to include equipe_chantier in team daily cost
...
DECLARE
  ...
  _etat_chantier TEXT;
BEGIN
  -- Budget depuis devis actif ou chantier + récupération de l'état
  SELECT 
    COALESCE(
  ...
  -- Coût journalier équipe depuis equipe_chantier (assignation d'une équipe complète)
  -- On considère une journée "standard" de 8h par membre, avec les mêmes règles de charges
  SELECT COALESCE(SUM(
    m.taux_horaire * 8 *
  ...
  -- Coût journalier total de l'équipe (affectations détaillées + équipes complètes)
